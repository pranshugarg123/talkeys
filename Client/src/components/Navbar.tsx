...
const Navbar = () => {
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.link}
            className={`flex ${
              isMobileView ? "flex-row items-center gap-3" : "flex-col items-center justify-center"
  ...
  const AuthButton = ({ isMobileView = false }) =>
    isSignedIn ? (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className="flex flex-col items-center cursor-pointer">
            ...
            <span className="mt-1 text-white text-sm">{firstName}</span> {/* Added First Name Below Avatar */}
          </div>
        </DropdownMenuTrigger>
      ...
      <Button asChild variant="outline" className="text-white hover:text-black hover:bg-white">
        <Link href="/sign">Login</Link>
      </Button>
    );
  ...
  return (
    <div className="fixed top-0 w-full z-[1000]">
      <div className="flex px-2.5 sm:px-5 justify-between bg-black items-center">
        ...
        {isMobile ? (
          <Button variant="default" size="icon" className="text-white" onClick={toggleMenu}>
            {isMenuOpen ? <X size={28} /> : <Menu size={28} />}
          </Button>
        ) : (
          <NavigationMenu>
